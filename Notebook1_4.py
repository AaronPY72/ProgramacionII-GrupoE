...
def guardarArchivo():
    with open("paciente.txt","w",encoding="utf-8") as archivo:
        for paciente in paciente_data:
            archivo.write(f"{paciente['Nombre']}|{paciente['FechaN']}|{paciente['Edad']}|{paciente['Genero']}|{paciente['GrupoS']}|{paciente['TipoS']}|{paciente['CentroM']}\n")
def guardarArchivoD():
    with open("doctores.txt","w",encoding="utf-8") as archivo:
        for doctores in doctores_data:
            archivo.write(f"{doctores['NombreD']}|{doctores['Especialidad']}|{doctores['EdadD']}|{doctores['Telefono']}\n")
...
def al_cambiar_pestaña():
    pestaña_activa=pestanas.index(pestanas.select())
    if pestaña_activa==0:
...
#boton Eliminar
btn_eliminar=tk.Button(btn_frame,text="Eliminar",command=eliminarDoctores,bg="red",fg="white")
...
cargarArchivoPaciente()
cargarArchivoDoctores()
""" pestanas.bind("<<NotebookTabChanged>>",al_cambiar_pestaña) """
=======
from datetime import date, datetime
from pydoc import doc
import tkinter as tk
...
cargarArchivoDoctores()
""" pestanas.bind("<<NotebookTabChanged>>",al_cambiar_pestaña) """
>>>>>>> 2c0127df65a490eb92d4ced06da40915b76a3e4a
